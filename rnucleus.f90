...
!
      ...
      REAL (8), DIMENSION(120,300) :: RRMSVEC
      INTEGER , DIMENSION (120)    :: amin, amax
      INTEGER :: i,j,m, n, nmax

      rrmsvec = 0.d0
      call rdata
      
      amin = 0; amax = 0; nmax=0

      Do i = 1, 120
         Do j = 1, 300
            if (rrmsvec(i,j) .ne. 0) then
                amin(i)= j
                exit
            end if
          end do
          ...
          n= amax(i) -amin(i) +1
          nmax = max(n,nmax)
          Print *, 'Z = ', i, 'A = ', amin(i), ' to ', amax(i), &
              'n =',  amax(i)-amin(i)+1
      end do
   
      Print *, 'Testing ...', rrmsvec(1,1:3)
      Print *, 'Maximum range = ', nmax

      Print 100, 'Data a_min /', amin
      Print 100, 'Data a_min /', amax
100      FORMAT(4x,A, 16(I3,',') // (8X,18(I3,',')//)) 


 
!   now let's output data statements (initially to the screen)

     Do i = 1,120
       If (amax(i) == 0 ) cycle
       n= amax(i) -amin(i) +1
 200 FORMAT(4x,A,I3,A,I2,A,8(F6.4,',') // (18X,9(F6.4, ',') //)) 
       Print 200, 'Data rr(', i,', 1:',n,') / ',rrmsvec(i,amin(i): amax(i))
     end Do
     STOP
